  ...
  return (
    <div
      className={
        compact
          ? 'grid gap-3'
          : 'grid gap-4 w-full bg-card rounded-[14px] px-6 py-8 border border-line'
      }
    >
      {!compact && (
      ...
      >
        <input
          ...
          } bg-white border border-line rounded-xl px-5 py-3 font-sans text-plum placeholder:text-slate/50 focus:outline-none focus:border-rose transition-colors duration-200`}
        />
        <button
          type="submit"
          ...
          } rounded-xl py-3 text-[0.95rem] font-semibold text-white no-underline transition-transform duration-200 hover:-translate-y-px disabled:opacity-60 disabled:cursor-not-allowed`}
          style={{
            background: '#cc6c4e',
      ...
      {status === 'success' ? (
        ...
        <p
          className={`${
            compact ? 'text-[0.82rem]' : 'text-sm'
          } text-rose border border-line bg-stone rounded-2xl px-4 py-2.5 ${
            compact ? '' : 'text-center'
          }`}
        >
          {dict.error}
