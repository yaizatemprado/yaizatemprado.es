...
const SLUG = 'presencia-ejecutiva'
const PATH = `recursos/${SLUG}`

type Props = { params: { locale: string } }

// Published in whichever locales have a copy (Spanish only for now).
...
export async function generateMetadata({ params: { locale } }: Props): Promise<Metadata> {
  ...
  if (!article) return {}
  // The H1 is the scene; the <title> is the term people type into Google.
  const title = `${article.seoTitle ?? article.title} — Yaiza Temprado`
  return {
    title,
    description: article.description,
    alternates: alternates(locale, PATH, localesOf(SLUG)),
    openGraph: { ...openGraph(locale, title, article.description, PATH, article.socialImage), type: 'article' },
  }
}
  ...
  const schema = [
    {
      ...
      ...(article.socialImage && { image: `${SITE_URL}${article.socialImage.src}` }),
